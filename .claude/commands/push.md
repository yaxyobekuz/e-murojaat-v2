...
For each repo (`client/`, `server/`, `admin/`) **sequentially**:
...
3. If there **are** changes, push them.
   - Do not ship everything in a single commit; split the changes into logical chunks and commit each one separately.
     - For admin: `git add <file1>` then `git commit -m "feat: add tournament filter"`, then `git add <file2>` then `git commit -m "fix: login form error"`.
