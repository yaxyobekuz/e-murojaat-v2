  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between text-[11px]">
        <span className="text-white/60">Agar yana ekilsa:</span>
      ...
      <div className="mt-1 flex justify-between text-[9px] text-white/30"><span>0</span><span>50 000</span></div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {[
          ...
          <div key={i} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-2.5 text-center">
            <c.icon className="mx-auto size-4" style={{ color: accent }} />
            <div className="mt-1.5 font-mono text-[13px] font-bold tabular-nums text-white">{c.value}</div>
            <div className="text-[9px] text-white/40">{c.label}</div>
          </div>
        ))}
      </div>
      ...
      <motion.div key={newCoverage} initial={{ opacity: 0.4 }} animate={{ opacity: 1 }}
        ...
        Ko'kalamzorlik <span className="font-bold text-white">{baseCoverage}%</span> →{" "}
        <span className="font-mono font-bold" style={{ color: accent }}>{newCoverage}%</span>
        {" "}· taxminan <span className="font-bold text-white">{mahallas}</span> mahalla qamrovi
      </motion.div>
    </div>
  );
};
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-white/60">Prognoz yili</span>
      ...
      <div className="mt-1 flex justify-between text-[9px] text-white/30">
        {[2026, 2027, 2028, 2029, 2030].map((y) => <span key={y} className={year === y ? "font-bold text-white/70" : ""}>{y}</span>)}
      </div>

      <div className="mt-4 grid grid-cols-2 gap-2">
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 text-center">
          <div className="font-mono text-2xl font-bold tabular-nums" style={{ color: accent }}>{coverage}%</div>
          <div className="text-[10px] text-white/45">Ko'kalamzorlik</div>
          <div className="mt-1 text-[9px] text-white/30">maqsad: {target}%</div>
        </div>
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 text-center">
          <div className="font-mono text-2xl font-bold tabular-nums text-white">{treesCum.toLocaleString("uz-UZ")}</div>
          <div className="text-[10px] text-white/45">Ekilgan ko'chat (küm.)</div>
          <div className="mt-1 text-[9px] text-white/30">{year} yilga</div>
        </div>
      </div>
