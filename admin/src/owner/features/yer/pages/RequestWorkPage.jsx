    ...
    <div className="mx-auto max-w-5xl px-6 py-6">
      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        {!isTerminal && (
          <Button
            onClick={() =>
              openModal(MODAL.YER_REQUEST_STATUS, {
                requestId: req._id,
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <Card title="Bog'liq obyekt">
          {req.propertyId ? (
            <div className="divide-y">
              <Row label="Kadastr raqami" value={req.propertyId.cadastreNumber} />
              <Row
                label="Tur"
                value={PROPERTY_TYPE_LABELS[req.propertyId.type] || req.propertyId.type}
              />
              <Row label="Manzil" value={req.propertyId.address} />
              <Row
                label="Maydon"
                value={req.propertyId.areaM2 ? `${req.propertyId.areaM2} m²` : "-"}
              />
            </div>
          ) : (
            <p className="text-sm text-zinc-400">Obyekt biriktirilmagan</p>
          )}
        </Card>
      </div>
      ...
      <ModalWrapper name={MODAL.YER_REQUEST_STATUS} title="Holatni o'zgartirish">
        <UpdateStatusModal />
      </ModalWrapper>
    </div>
  );
