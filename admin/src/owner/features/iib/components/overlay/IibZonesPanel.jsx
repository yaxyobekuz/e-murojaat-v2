// Xarita ustida suzuvchi bloklar paneli — qatorni bossang xarita o'sha blokka
// uchadi va kamera yozuvlari ochiladi (onSelect).
import { Video, ShieldAlert } from "lucide-react";

import { cn } from "@/shared/utils/cn";
import { SECURITY_STATUS } from "../../mock/iib.mapAreas";

const IibZonesPanel = ({ zones, activeId, onSelect }) => (
  <div className="surface-overlay flex max-h-[260px] w-72 flex-col rounded-xl p-2">
    ...
    <div className="flex flex-col gap-0.5 overflow-y-auto pr-0.5">
      {zones.length === 0 && (
        <p className="px-2 py-4 text-center text-xs text-foreground/40">Blok yo'q</p>
      )}
      {zones.map((a) => {
        const st = SECURITY_STATUS[a.status];
          ...
          >
            ...
            {a.info.incidents > 0 && (
              <span className="flex shrink-0 items-center gap-0.5 text-[11px] font-semibold text-rose-400">
                <ShieldAlert className="size-3" /> {a.info.incidents}
              </span>
            )}
