...
import { Users, ShieldCheck, Activity, Clock, Camera, MapPin, Shield } from "lucide-react";
...
const MAP3D_MARKERS = [
  { lat: 40.6402, lng: 72.2378, label: "MFY profilaktika posti", status: "Navbatchi: 2 xodim" },
  { lat: 40.6385, lng: 72.2405, label: "Kamera — Markaziy ko'cha", status: "Jonli efir" },
  { lat: 40.6371, lng: 72.2362, label: "Hodisa — Bozor", status: "Tergovda" },
  { lat: 40.6418, lng: 72.2418, label: "Kamera — Maktab oldi", status: "Jonli efir" },
  { lat: 40.636, lng: 72.239, label: "Patrul nuqtasi", status: "Faol" },
];
...
const CITIZENS = Array.from({ length: 9 }, (_, i) => {
  ...
  const status = pick(["Faol", "Faol", "Faol", "Nazoratda", "Yangi ko'chib kelgan"]);
...
const CAM_LOCS = ["Markaziy ko'cha", "MFY binosi oldi", "Bozor kirishi", "Maktab oldi", "Bolalar maydoni", "Avtoturargoh"];
...
const TYPE_COLORS = { "O'g'irlik": "#22d3ee", Bezorilik: "#a78bfa", YTH: "#fbbf24", "Tan jarohati": "#fb7185", Boshqa: "#38bdf8" };
...
const STATUS_DIST = [
  ...
  { name: "Tergovda", value: ri(20, 60), color: "#fbbf24" },
  { name: "Ochiq", value: ri(8, 30), color: "#fb7185" },
];
const AGE_GROUPS = ["0-17", "18-29", "30-44", "45-59", "60-74", "75+"];
const RADAR_MALE = AGE_GROUPS.map(() => ri(10, 95));
const RADAR_FEMALE = AGE_GROUPS.map(() => ri(10, 95));
const GRID_CELLS = TYPES.flatMap((row) => DAY_LABELS.map((col) => ({ row, col, value: ri(0, 14) })));

const MAP_MARKERS = [
  ...CAMERAS.map((c, i) => ({ id: c.id, x: 12 + ((i * 14.5) % 76), y: 22 + ((i * 23) % 56), type: "camera", label: `${c.id} · ${c.loc}`, status: c.online ? "Jonli efir" : "Signal yo'q" })),
  { id: "inc1", x: 38, y: 40, type: "incident", label: "Bezorilik — Markaziy ko'cha", status: "Tergovda" },
  { id: "inc2", x: 64, y: 58, type: "incident", label: "O'g'irlik — Bozor", status: "Ochiq" },
  { id: "inc3", x: 26, y: 66, type: "incident", label: "YTH — Navoiy ko'chasi", status: "Bartaraf etilgan" },
  { id: "post", x: 50, y: 34, type: "unit", label: "MFY profilaktika posti", status: "Navbatchi: 2 xodim" },
];
...
function nestedRadialOption() {
  return {
    ...
    polar: { radius: ["25%", "42%"] },
    series: [
      { type: "bar", coordinateSystem: "polar", data: [Math.round((KPIS.active / 7) * 100)], roundCap: true, itemStyle: { color: "#22d3ee" }, barWidth: 14 },
      {
        type: "pie",
        radius: ["58%", "80%"],
        center: ["50%", "50%"],
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-semibold tracking-tight">
            <Shield className="size-5 text-cyan-400" /> {MAHALLA.name} — Xavfsizlik tahlili
          </h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <GlassStatCard label="Jinoyatlar (yillik)" value={KPIS.crimesYear} icon={Activity} accent="purple" />
        <GlassStatCard label="Ochilish darajasi" value={KPIS.clearance} suffix="%" icon={ShieldCheck} accent="emerald" glow />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Section title="Fuqarolar — yosh va jins" icon={Users} accent="cyan">
          <EChart option={radarOpt} height={290} />
        </Section>
      </div>

      {/* bubble grid */}
      <Section title="Jinoyat turi × hafta kuni" icon={Activity} accent="purple">
        <BubbleGridHeatmap rows={TYPES} cols={DAY_LABELS} cells={GRID_CELLS} accent="#7c6cf0" />
      </Section>

      {/* 3D map + 2D map */}
      <div className="grid gap-4 xl:grid-cols-2">
        <Section title="Photorealistik 3D xarita" icon={MapPin} accent="indigo">
          <Mahalla3DMap markers={MAP3D_MARKERS} height={340} fallbackSeed={424242} />
        </Section>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Section title="Jinoyat turlari bo'yicha" icon={Activity} accent="purple"><EChart option={typeBreak} height={220} /></Section>
      ...
      {/* citizens */}
      <Section title="Fuqarolar ma'lumotlari" icon={Users} accent="cyan">
