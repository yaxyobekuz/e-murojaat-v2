    ...
    <div className="mx-auto max-w-5xl px-6 py-6">
      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        {!isTerminal && (
          <Button
            onClick={() =>
              openModal(MODAL.GAZ_REQUEST_STATUS, {
                requestId: req._id,
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Card title="Ariza ma'lumotlari">
          <div className="divide-y">
            ...
            {req.operatorNote && <Row label="Operator izohi" value={req.operatorNote} />}
          </div>
        </Card>

        <Card title="Bog'liq abonent">
          {sub ? (
            <div className="divide-y">
              <Row label="Hisob raqami" value={sub.accountNumber} />
              <Row label="Abonent" value={sub.fullName} />
              <Row label="Turi" value={SUBSCRIBER_TYPE_LABELS[sub.type] || sub.type} />
              <Row label="Manzil" value={`${sub.region}, ${sub.address || ""}`} />
            </div>
          ) : (
      ...
      <ModalWrapper name={MODAL.GAZ_REQUEST_STATUS} title="Holatni o'zgartirish">
        <UpdateStatusModal />
      </ModalWrapper>
    </div>
  );
