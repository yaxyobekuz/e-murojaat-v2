// FVV operativ xarita — IIB uslubidagi bloklar + ko'cha yo'llari. Blok (yong'in)
// tanlanganda — yaqin atrofdagi kameralarni kuzatish paneli ochiladi.
import { useMemo } from "react";

import useObjectState from "@/shared/hooks/useObjectState";
import { MAHALLA_AREAS, MAP_PLACE_LABEL, summarize } from "../mock/fvv.mapAreas";
...
import FvvZonesPanel from "../components/overlay/FvvZonesPanel";

const FvvMapPage = () => {
  const { statusFilter, activeId, setField } = useObjectState({ statusFilter: [], activeId: null });

  const toggleStatus = (key) =>
    setField(
      "statusFilter",
      statusFilter.includes(key) ? statusFilter.filter((s) => s !== key) : [...statusFilter, key],
    );
  ...
  return (
    <div className="relative h-[calc(100vh-7rem)] w-full overflow-hidden rounded-2xl border border-[rgb(var(--card-border))] bg-card">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 flex justify-start p-4">
        <div className="pointer-events-auto">
          <FvvZonesPanel zones={zones} activeId={activeId} onSelect={(id) => setField("activeId", id)} />
        </div>
      </div>
